/***************************************************************************//**
 * @file win32_StoreLogo_png.cpp
 * @author StoneyDSP (nathanjhood@googlemail.com)
 *
 * @brief Contains a default definition of 'StoreLogo_png_array' (see
 * 'copyright' section).
 *
 * @version 1.0.0
 * @date 2023-08-24
 *
 * @copyright This file was generated by using Vector-of-Bool's excellent
 * 'CMakeRC' resource compiler script from CMake's template resources for
 * Windows. The results have been given minimal tweaks - mostly just text
 * formatting - to provide a similar interface to the rest of the project's
 * codebase. Please see 'cmrc.hpp' and Kitware.org for respective license
 * information.
 *
 * @see 'StoreLogo.png'
 *
 ******************************************************************************/

#include "win32_StoreLogo_png.hpp"

namespace
{
/**
 * @brief An array of bytes representing 'StoreLogo.png'.
 *
 */
RCRESOURCE StoreLogo_png_array[228] = {
    '\x89', '\x50', '\x4e', '\x47', '\x0d',
    '\x0a', '\x1a', '\x0a', '\x00', '\x00',
    '\x00', '\x0d', '\x49', '\x48', '\x44',
    '\x52', '\x00', '\x00', '\x00', '\x32',
    '\x00', '\x00', '\x00', '\x32', '\x08',
    '\x04', '\x00', '\x00', '\x00', '\xb4',
    '\x36', '\x40', '\x3a', '\x00', '\x00',
    '\x00', '\xaa', '\x49', '\x44', '\x41',
    '\x54', '\x78', '\xda', '\xed', '\xd7',
    '\xb5', '\x01', '\x43', '\x31', '\x0c',
    '\x05', '\x40', '\x0f', '\xa3', '\xfd',
    '\x1b', '\x0d', '\x90', '\xd1', '\x94',
    '\xf2', '\x77', '\xbe', '\xe0', '\x0b',
    '\xca', '\x5d', '\x40', '\x67', '\x86',
    '\xf5', '\x8f', '\x1f', '\x8d', '\x39',
    '\xca', '\xe9', '\xb1', '\xe5', '\xc8',
    '\xfc', '\x22', '\xa4', '\xa7', '\x66',
    '\xdd', '\x51', '\x6a', '\x5a', '\x48',
    '\x8f', '\x19', '\x13', '\xbd', '\x47',
    '\x6a', '\xcc', '\x98', '\xa8', '\x3d',
    '\xb2', '\xc6', '\x8c', '\x89', '\x05',
    '\xc4', '\x8c', '\x09', '\x22', '\x64',
    '\x4c', '\x18', '\x21', '\x63', '\xc2',
    '\x08', '\x19', '\x13', '\x46', '\xc8',
    '\x98', '\x30', '\x82', '\x04', '\x26',
    '\x8c', '\x38', '\x89', '\xbf', '\x35',
    '\xe2', '\x44', '\xfe', '\xc6', '\x88',
    '\x19', '\x13', '\x46', '\xcc', '\x98',
    '\x30', '\x62', '\x86', '\x84', '\x11',
    '\x32', '\x26', '\x8c', '\x90', '\x31',
    '\x61', '\x84', '\x8c', '\x89', '\x3c',
    '\x92', '\xef', '\xae', '\xfc', '\xc0',
    '\xe7', '\xa7', '\xb0', '\x17', '\x63',
    '\x7e', '\x5b', '\xc9', '\x6f', '\x90',
    '\xf9', '\xad', '\x3e', '\x7e', '\x68',
    '\x21', '\x81', '\x99', '\xfc', '\x45',
    '\x22', '\x7f', '\x25', '\xca', '\x5e',
    '\xee', '\x4c', '\x98', '\xc9', '\x5f',
    '\xb8', '\x73', '\x4f', '\x87', '\xc0',
    '\x23', '\x28', '\xff', '\x9c', '\xfb',
    '\xe0', '\xd7', '\xef', '\x3f', '\x7e',
    '\x33', '\xce', '\x86', '\xde', '\x52',
    '\x58', '\xe8', '\x57', '\x07', '\x3e',
    '\x00', '\x00', '\x00', '\x00', '\x49',
    '\x45', '\x4e', '\x44', '\xae', '\x42',
    '\x60', '\x82', 0 };
}

namespace CMakeRC
{
namespace Img
{
namespace StoreLogo
{
/** @brief Pointer to the beginning of 'StoreLogo_png_array'. */
extern RCRESOURCEPTR const StoreLogo_png_begin = ::StoreLogo_png_array;
/** @brief Pointer to the end of 'StoreLogo_png_array'. */
extern RCRESOURCEPTR const StoreLogo_png_end = ::StoreLogo_png_array + 227;
} // namespace StoreLogo
} // namespace Img
} // namespace CMakeRC
